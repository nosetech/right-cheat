...
アプリケーションの初回起動時にブロックされる(または、「ファイルが壊れています」というメッセージが表示される)場合があります。その際は以下のいずれかの手順を実施してください：
...
1. ターミナルを開きます
2. 以下のようにダウンロードしたdmgファイルに対してxattrコマンドを実行します

```
xattr -cr RightCheat*.dmg
```
