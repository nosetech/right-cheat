...
- **キーボード操作**:
  ...
  - Enter: コピー実行
  - 1～9、0: 数字キーで該当のコマンドをコピー（数字順に表示されるコマンド）

### チートシート切り替え

- **マウス操作**: メインウィンドウ上部のドロップダウンリストから表示するチートシートを選択
- **キーボード操作**: 1～9、0キーで該当のチートシートを選択（チートシート一覧の順番に対応）
...
メニュー [View] → [Light/Dark] でアプリケーションのテーマを切り替えることができます。
...
コマンドの description フィールドにマウスを重ねると、ツールチップ（説明文）が表示されます。

- 長い説明文が見切れている場合に全文を確認可能
- キーボード操作時にも説明文がクリアに表示されます
