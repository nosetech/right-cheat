...
#### Cargo.toml の name フィールドについて

`src-tauri/Cargo.toml` の `name` は **`RightCheat`（大文字混じり）** を使用している。
...
Rust の慣習ではパッケージ名に小文字（`right-cheat`）を推奨しているが、macOS の開発実行時（`yarn tauri dev`）はバンドルされた `.app` ではなくバイナリ直接実行となるため、Cargo のパッケージ名がプロセス名としてシステム設定のアクセシビリティ一覧に登録される。`right-cheat` のままではアクセシビリティ設定画面で正しいアプリ名が表示されないため、意図的に `RightCheat` としている。
