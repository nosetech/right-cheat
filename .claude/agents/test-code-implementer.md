...
### このプロジェクト（RightCheat）の Rust テスト配置ルール

- **全ての Rust ユニットテストは `src-tauri/tests/` 配下に配置すること**
- ソースファイル（`src-tauri/src/`）内への `#[cfg(test)]` インラインテストは **使用しない**
- テスト対象に応じてディレクトリを分ける：
  - API レイヤーのテスト → `src-tauri/tests/api/<module>.rs`
  - DB レイヤーのテスト → `src-tauri/tests/db/<module>.rs`
  - 新モジュールを追加する場合は `tests/lib.rs` と対応する `mod.rs` にも登録すること
- テストファイルからは `app_lib::` パスで本番コードを参照する（`super::*` は使わない）
